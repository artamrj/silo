...
import { intHash, isDev } from "../shared/utils";
...
class Logger {
    ...
    /**
     * SILO_HIDE_LOG=debug_monitor,info_monitor
     *
     * Example:
     *  [
     ...
     */
    constructor() {
        if (typeof process !== "undefined" && process.env.SILO_HIDE_LOG) {
            const list = process.env.SILO_HIDE_LOG.split(",").map(v => v.toLowerCase());
            ...
            this.debug("server", "SILO_HIDE_LOG is set");
